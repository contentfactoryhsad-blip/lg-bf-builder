...
import {
  ALPHA_MIN, BackdropThresholds, backdropThresholds, edgeCoverage, morph,
} from "../utils/backdrop";
...
/**
 * Radius, in native pixels, of the closing applied to a smart-fill selection.
 * The colour test leaves pinholes and hairline nicks all through a region that
 * reads as one flat area to the eye; closing swallows anything narrower than
 * twice this, so a click selects the blob a person meant to point at.
 */
const SMART_CLOSE_R = 2;
...
export function BrushMaskEditor({ originalUrl, processedUrl, onDone, onCancel }: Props) {
  ...
  // Backdrop level of the ORIGINAL shot, measured once on load (see ./utils/backdrop).
  const backdropRef = useRef<BackdropThresholds | null>(null);
  ...
  useEffect(() => {
    ...
    Promise.all([loadImg(originalUrl), loadImg(processedUrl)])
      .then(([orig, proc]) => {
        ...
        backdropRef.current = backdropThresholds(originalDataRef.current.data, cw, ch);
  ...
  // ── Smart Flood Fill ─────────────────────────────────────────────────────────
  /**
   * Flood fill from (seedX, seedY) using colour similarity in the ORIGINAL image.
   ...
   *
   * The original image is always used as the colour reference so the fill
   * boundary follows the real, un-modified pixel colours (not what the user may
   * have already partially erased).
   *
   * Scanline rather than per-pixel, and with no pixel budget. The old version
   * pushed one stack entry per pixel and stopped at 600k, which a 1044×1334
   * product shot blows past — its white surround matches 968k pixels. Stopping
   * there left the rest of the region untouched, and because a per-pixel stack
   * walks a grid as a snake, the boundary where it stopped came out as a run of
   * VERTICAL STRIPES down the unfinished side. Claiming whole horizontal runs at
   * a time is both fast enough to need no budget and, if one were ever added,
   * coherent about where it stops.
   */
  const floodFillAt = useCallback((seedX: number, seedY: number): number => {
    ...
    // A click on the studio backdrop is judged against the level measured off
    // the frame, exactly as the automatic pass judges it — otherwise the brush
    // would undo that work on the first click. The fixed tolerance reaches 245
    // from a 255 seed, which is inside a white product's own tonal range, so it
    // would eat back the specular highlight the automatic pass took care to
    // keep. Anywhere else — restoring a product, clearing a coloured corner —
    // there is no backdrop to measure against and the tolerance still rules.
    const bd = backdropRef.current;
    const minCh = (pos: number) => {
      const p = pos * 4;
      return Math.min(ox[p], ox[p + 1], ox[p + 2]);
    };
    const onBackdrop = bd !== null && bd.level !== null && minCh(sy * w + sx) >= bd.hi;

    const like = onBackdrop
      ? (pos: number) => ox[pos * 4 + 3] < ALPHA_MIN || minCh(pos) >= bd!.hi
      : (pos: number) => {
        const ni = pos * 4;
        const dr = ox[ni] - sr, dg = ox[ni + 1] - sg, db = ox[ni + 2] - sb;
        return dr * dr + dg * dg + db * db <= tSq;
      };
    if (!like(sy * w + sx)) return 0;

    // Scanline fill: each stack entry is one seed, and popping it claims the
    // whole horizontal run around it, then seeds only the runs above and below.
    let sel: Uint8Array = new Uint8Array(w * h);
    ...
    while (stack.length > 0) {
      ...
      let l = pos % w;
      let r = l;
      while (l > 0 && !sel[row + l - 1] && like(row + l - 1)) l--;
      while (r < w - 1 && !sel[row + r + 1] && like(row + r + 1)) r++;
      for (let x = l; x <= r; x++) sel[row + x] = 1;
      // Seed the neighbouring rows once per contiguous run, not once per pixel.
      for (const ny of [y - 1, y + 1]) {
        if (ny < 0 || ny >= h) continue;
        const nrow = ny * w;
        let inRun = false;
        for (let x = l; x <= r; x++) {
          const ok = !sel[nrow + x] && like(nrow + x);
          if (ok && !inRun) stack.push(nrow + x);
          inRun = ok;
        }
      }
    }

    // Close the selection so it comes out as one blob. The colour test riddles a
    // flat area with pinholes — JPEG speckle, a faint reflection, the shadow of
    // a seam — and without this the user has to click the same region a dozen
    // times to clear what looked like one patch.
    sel = morph(morph(sel, w, h, SMART_CLOSE_R, "dilate"), w, h, SMART_CLOSE_R, "erode");

    // Erasing the backdrop gets the same sub-pixel edge the automatic pass
    // computes, rather than a hard alpha=0 — otherwise a click leaves a
    // staircase along exactly the outline the automatic pass got smooth, and
    // whether an edge looks cut depends on which tool last touched it.
    const kept = new Uint8Array(w * h);
    for (let pos = 0; pos < w * h; pos++) kept[pos] = sel[pos] ? 0 : 1;
    const cover = onBackdrop ? edgeCoverage(ox, kept, w, h, bd!.level) : null;
    ...
    for (let pos = 0; pos < w * h; pos++) {
      ...
      if (mode === "erase") {
        if (cover) {
          // The band around the selection is partly product, so lower its alpha
          // rather than clearing it; never raise what the user already erased.
          const a = Math.round(cover[pos] * ox[i + 3]);
          if (a >= px[i + 3]) continue;
          px[i + 3] = a;
        } else {
          if (!sel[pos]) continue;
          px[i + 3] = 0;
