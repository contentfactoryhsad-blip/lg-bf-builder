// Product cut-outs for the Content Template Builder's PD Slot plates.
//
// 🔴 This is the Sales Banner Builder's background remover, copied verbatim
// (`07 sales template/sales-banner-builder/src/app/utils/aiBgRemoval.ts`).
// The PD Slot flow was specified as "same as the Sales Banner Builder", and
// that has to include the cut-out itself — this repo's own
// `brandshop/modules/aiBgRemoval` is a later, different algorithm (it measures
// the backdrop off the frame edge instead of using a fixed white threshold, and
// solves sub-pixel alpha) and it produced visibly worse edges on the LG.com
// packshots this builder feeds on.
//
// Keep it byte-compatible with the Sales Banner Builder rather than "improving"
// it here: the point is that the two tools cut the same photo the same way.
// The other builders in this repo still use `brandshop/modules/aiBgRemoval`;
